    ...
    <div className="w-full max-w-[1100px] mx-auto flex flex-col gap-16 p-6 lg:p-12">
      ...
      <section className="flex flex-col items-center text-center gap-10 mt-8 lg:mt-12">
        ...
        <form onSubmit={handleSearch} className="w-full max-w-3xl relative group rounded-full transition-all transform hover:scale-[1.01]">
          ...
          <span className="material-symbols-outlined absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary text-2xl z-10" aria-hidden="true">search</span>
          <input
            id="search-input"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            aria-label="Pesquisa de termos técnicos"
            className="w-full bg-night-panel/80 border border-night-border text-slate-100 pl-16 pr-48 py-4 rounded-full text-lg focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary shadow-2xl transition-all placeholder:text-slate-500 font-display"
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
            ...
            >
              <span className="material-symbols-outlined text-lg icon-filled" aria-hidden="true">mic</span>
            </button>
          </div>
        </form>
